...
export function useChatList(): ChatListProps {
  ...
  const createChat = useCallback(async (name: string, assistant_id: string) => {
    const response = await fetch(`${process.env.VITE_BACKEND_URL}//threads`, {
      method: "POST",
  ...
  const updateChat = useCallback(
    async (thread_id: string, name: string, assistant_id: string | null) => {
      const response = await fetch(`${process.env.VITE_BACKEND_URL}//threads/${thread_id}`, {
        method: "PUT",
        body: JSON.stringify({ assistant_id, name }),
        headers: {
          "Content-Type": "application/json",
  ...
  const deleteChat = useCallback(
    async (thread_id: string) => {
      await fetch(`${process.env.VITE_BACKEND_URL}//threads/${thread_id}`, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          "Authorization": `Bearer ${localStorage.getItem("token")}`,
